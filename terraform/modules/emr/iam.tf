...
data "aws_iam_policy_document" "external_batch_metadata_change" {
  statement {
    effect = "Allow"
...
resource "aws_iam_policy" "external_batch_metadata_change" {
  name        = "AnalyticalEnvMetadataOptions"
  description = "Allow editing of Metadata Options"
  policy      = data.aws_iam_policy_document.external_batch_metadata_change.json
}

resource "aws_iam_role_policy_attachment" "external_batch_metadata_change" {
  role       = aws_iam_role.emr_ec2_role.name
  policy_arn = aws_iam_policy.external_batch_metadata_change.arn
}
