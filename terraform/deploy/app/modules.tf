data "aws_secretsmanager_secret_version" "hive_metastore_password_secret" {
  provider  = aws
  secret_id = "metadata-store-v2-external-batch"
}
...
module "emr" {
  ...
  interface_vpce_sg_id    = data.terraform_remote_state.dataworks_aws_external_batch_infra.outputs.interface_vpce_sg_id
  s3_prefix_list_id       = data.terraform_remote_state.dataworks_aws_external_batch_infra.outputs.s3_prefix_list_id
  dynamodb_prefix_list_id = data.terraform_remote_state.dataworks_aws_external_batch_infra.outputs.dynamodb_prefix_list_id
  internet_proxy_dns_name = data.terraform_remote_state.dataworks_aws_external_batch_infra.outputs.internet_proxy_dns_name
  internet_proxy_sg_id    = data.terraform_remote_state.dataworks_aws_external_batch_infra.outputs.internet_proxy_sg
  ...
  vpc                     = data.terraform_remote_state.dataworks_aws_external_batch_infra.outputs.vpc
...
module "pushgateway" {
  ...
  name_prefix = "external-batch-pushgateway"

  container_name       = "prom-pushgateway"
  image_ecr_repository = data.terraform_remote_state.management.outputs.ecr_pushgateway_url

  subnets              = data.terraform_remote_state.dataworks_aws_external_batch_infra.outputs.vpc.aws_subnets_private[*].id
  vpc_id               = data.terraform_remote_state.dataworks_aws_external_batch_infra.outputs.vpc.aws_vpc.id
  interface_vpce_sg_id = data.terraform_remote_state.dataworks_aws_external_batch_infra.outputs.interface_vpce_sg_id
  s3_prefixlist_id     = data.terraform_remote_state.dataworks_aws_external_batch_infra.outputs.s3_prefix_list_id
  logging_bucket       = data.terraform_remote_state.security-tools.outputs.logstore_bucket.id
...
module "livy_proxy" {
  ...
  vpc_id               = data.terraform_remote_state.dataworks_aws_external_batch_infra.outputs.vpc.aws_vpc.id
  subnet_ids           = data.terraform_remote_state.dataworks_aws_external_batch_infra.outputs.vpc.aws_subnets_private[*].id
  s3_prefix_list_id    = data.terraform_remote_state.dataworks_aws_external_batch_infra.outputs.s3_prefix_list_id
  interface_vpce_sg_id = data.terraform_remote_state.dataworks_aws_external_batch_infra.outputs.interface_vpce_sg_id
...
module "launcher" {
  ...
  dataworks_aws_external_batch_emr_launcher_zip = var.dataworks_aws_external_batch_emr_launcher_zip
  ami                                           = var.emr_al2_ami_id
  log_bucket                                    = data.terraform_remote_state.security-tools.outputs.logstore_bucket.id
  account                                       = local.account[local.environment]
  external_batch_security_configuration         = module.emr.external_batch_security_configuration
  ...
  proxy_host                                    = data.terraform_remote_state.dataworks_aws_external_batch_infra.outputs.internet_proxy_dns_name
...
module "emrfs_lambda" {
  ...
  account                    = local.account[local.environment]
  aws_subnets_private        = data.terraform_remote_state.dataworks_aws_external_batch_infra.outputs.vpc.aws_subnets_private[*].id
  common_tags                = local.common_tags
  name_prefix                = "external-batch-emrfs-lambda"
  region                     = var.region
  vpc_id                     = data.terraform_remote_state.dataworks_aws_external_batch_infra.outputs.vpc.aws_vpc.id
  internet_proxy_sg_id       = data.terraform_remote_state.dataworks_aws_external_batch_infra.outputs.internet_proxy_sg
...
module "rbac_db" {
  ...
  name_prefix = "external-batch-rbac"
